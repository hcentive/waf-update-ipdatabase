...
class IPDatabase {
    ...
    return new Promise(function(resolve, reject) {
      var getItemCountPromise = db.getItemCount().then((c) => {
        ...
        batchWritePromise.then(function(results) {
          (function waitForItemCountMatch() {
            process.stdout.write('.');
            var getItemPromise = db.getItemCount().then((count) => {
              if (count < expectItemCount) {                
                waitForItemCountMatch();
              } else {
                resolve(items);
              }
  ...
  getItemCount() {
    ...
    return new Promise(function(resolve, reject) {
      var descTablePromise = db.dynamodb.describeTable({ TableName: db.tableName }).promise();
      descTablePromise.then((results) => {
        process.stdout.write('.');
        resolve(results.Table.ItemCount);
      }).catch((error) => reject(error));
    });
  }
}
