...
class IPDatabase {
  ...
  updateAddresses(ipaddresses, source, callback) {
    ...
    db.updateThroughput(ipaddresses.length, ipaddresses.length).then(function(results) {
      ...
      all.then(function(addrs) {
        ...
        db.waitForTableActive(db.tableName).then(function(d) {
            db.updateThroughput(10, db.existingTableWriteCapacity).then(function(d) {
            callback(null, addrs);
          }).catch(function(e) {
            callback(e, null);
          });
      ...
      describeTablePromise.then(function(data) {
        ...
        if (data.Table.ProvisionedThroughput.WriteCapacityUnits !== capacity) {
          ...
          db.waitForTableActive(db.tableName).then(function(d) {
            ...
            updPromise.then(function(re) {
              // console.log('table capacity updated. wait for it to become active.');
              db.waitForTableActive(db.tableName).then((res) => {console.log('table ACTIVE'); resolve(res);}).catch((err) => {console.log('wateFor error'); reject(err);});
            }).catch((er) => reject(er));
          }).catch(function(e) {
            reject(e);
          });
